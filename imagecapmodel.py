...
Original file is located at
    https://colab.research.google.com/drive/1ldcRd9OZusdWNf1eBrjWwgYelviZPEmX
"""
...
In this project we have defined and trained an image-to-caption model, that can produce descriptions for real world images.
...
We use pre-trained InceptionV3 model for CNN encoder and extract its last hidden layer as an embedding:
...
"""Features extraction takes too much time on CPU. The following code shortens the time period.
```python
...
# reset the graph if building from scratch
s = reset_tf_session()
...
dense_layer(a)  # Applying dense layer
dense_layer(b)  
```

Here's a figure to help flattening in decoder:
...
"""The training and validation losses are decreasing with each iteration"""
...
# check the model's learnings, outputs accuracy of next word prediction (should be around 0.5)
...
"""Images from the internet can be fed to the model for captioning"""

download_utils.download_file(
    "https://unsplash.com/photos/mY3_bvR74fI/download?ixid=MnwxMjA3fDB8MXxhbGx8fHx8fHx8fHwxNjQyMzIxMTE0&force=true",
    "portal.jpg"
)

apply_model_to_image_raw_bytes(open("portal.jpg", "rb").read())

"""Using images from validation set is as follows:
```python
show_valid_example(val_img_fns, example_idx=...)
```

Using images from the Internet is as follows:
```python
! wget ...
...
show_valid_example(val_img_fns, example_idx=44)

show_valid_example(val_img_fns, example_idx=184)

show_valid_example(val_img_fns, example_idx=212)

"""**10 Examples of Failure**"""

show_valid_example(val_img_fns, example_idx=5)

show_valid_example(val_img_fns, example_idx=16)

show_valid_example(val_img_fns, example_idx=24)

show_valid_example(val_img_fns, example_idx=40)

show_valid_example(val_img_fns, example_idx=45)

show_valid_example(val_img_fns, example_idx=85)

show_valid_example(val_img_fns, example_idx=125)

show_valid_example(val_img_fns, example_idx=120)

show_valid_example(val_img_fns, example_idx=199)
